...
public class CreateEllipse implements Command{
	...
	public CreateEllipse(Map<String, Preference> preferences, double mouseX, double mouseY, EllipseShape ellipse, GraphicsContext artBoard) {
		...
		newEllipse = ellipse;
		newEllipse.setCenterX(mouseX-preferences.get("width").getDoublePreference()/2);
		newEllipse.setCenterY(mouseY-preferences.get("radius").getDoublePreference()/2);
		newEllipse.setWidth(preferences.get("width").getDoublePreference());
	...
	public void execute() {
		...
		curArtBoard.fillArc(newEllipse.getCenterX(), newEllipse.getCenterY(), newEllipse.getWidth(), newEllipse.getRadius(), 0, 360, ArcType.OPEN);
